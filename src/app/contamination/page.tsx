...
async function getContaminationData() {
  ...
  return {
    ...
    stats: {
      ...
      contaminationRate:
        totalIntakesYTD > 0
          ? ((contaminationCountYTD / totalIntakesYTD) * 100).toFixed(1)
          : "0",
    },
    byType,
    bySeverity,
...
export default async function ContaminationPage() {
  ...
  return (
    <div>
      <Header
        title="Contamination Log"
        subtitle="Track and manage contamination incidents"
      />
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {data.stats.contaminationCountYTD}
                  </p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {data.stats.contaminationCountMonth}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {data.stats.totalIntakesYTD - data.stats.contaminationCountYTD}
                  </p>
                </div>
              ...
              {data.byType.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.byType.map((item) => (
                    <div
                      key={item.contaminantType}
                      className="flex items-center justify-between"
                    >
                      <span className="text-sm capitalize">
                        {item.contaminantType.replace(/_/g, " ")}
                      </span>
            ...
            {data.reports.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-400">
                  Great job keeping waste streams clean!
                </p>
              </div>
            ) : (
                    ...
                    {data.reports.map((report) => (
                      <tr key={report.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <Link
                            href={`/intakes/${report.intake.id}`}
                            className="font-mono text-sm text-emerald-600 hover:text-emerald-800"
                          >
                            {report.intake.ticketNumber}
                          </Link>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm">
                          {report.intake.client.companyName}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm">
                          {getContaminantLabel(report.contaminantType)}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span
                            className={`px-2 py-1 rounded text-xs font-semibold ${getSeverityColor(
          ...
          <CardContent>
            {data.contaminatedIntakes.length === 0 ? (
              <p className="text-gray-500 text-center py-4">
                No contaminated intakes this year
              </p>
            ) : (
              <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between p-3 bg-red-50 rounded-lg hover:bg-red-100 transition-colors">
                      ...
                      <div className="text-right">
                        <p className="text-sm text-gray-500">
                          {formatDate(intake.receivedAt || intake.scheduledDate)}
                        </p>
                      </div>
                    </div>
                  </Link>
