 ...
 */
export async function uploadScalePhoto(file: File | Blob, transactionId: string, type: "gross" | "tare"): Promise<string> {
  const fileName = `${transactionId}/${type}-${Date.now()}.jpg`;

  const { error } = await supabase.storage.from(STORAGE_BUCKETS.SCALE_PHOTOS).upload(fileName, file, {
  ...
  const { data: urlData } = supabase.storage.from(STORAGE_BUCKETS.SCALE_PHOTOS).getPublicUrl(fileName);
 ...
 */
export async function uploadSignature(base64Data: string, transactionId: string): Promise<string> {
  ...
  const { error } = await supabase.storage.from(STORAGE_BUCKETS.SIGNATURES).upload(fileName, blob, {
  ...
  const { data: urlData } = supabase.storage.from(STORAGE_BUCKETS.SIGNATURES).getPublicUrl(fileName);
 ...
 */
export async function uploadDocument(pdfBuffer: Buffer | Blob, transactionId: string, type: "weight-ticket" | "bol"): Promise<string> {
  const fileName = `${transactionId}/${type}-${Date.now()}.pdf`;

  const { error } = await supabase.storage.from(STORAGE_BUCKETS.DOCUMENTS).upload(fileName, pdfBuffer, {
  ...
  const { data: urlData } = supabase.storage.from(STORAGE_BUCKETS.DOCUMENTS).getPublicUrl(fileName);
 ...
 */
export async function deleteStorageFiles(bucket: keyof typeof STORAGE_BUCKETS, paths: string[]): Promise<void> {
